...
func (m *MyPlugin) PingTestTime() ([]timeplugin.TestTime, error) {
	...
	times, _ := time.ParseInLocation(timeLayout, "2023-06-18 12:50:18", time.Local)
	time2, _ := time.ParseInLocation(timeLayout, "2023-06-18 12:51:18", time.Local)

	t := []timeplugin.TestTime{
		{Start: times, Durtime: time.Second * 5},
		{Start: time2, Durtime: time.Second * 5},
	}
	return t, nil
}
